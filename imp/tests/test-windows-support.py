...
def test_powershell_keepalive():
    ...
    assert 'ensure-openssh' in data, 'imp-start.ps1 should prepare openssh tooling'
    assert 'convertto-json' in data, 'imp-start.ps1 should manage cluster node configuration'
    assert 'imp_remote_dir' in data, 'imp-start.ps1 should export the IMP_REMOTE_DIR path'
