...
def schedule_tasks():
    ...
    local_tasks = [
        [sys.executable, str(ROOT / "self-improvement" / "imp-code-updater.py")],
        [sys.executable, str(ROOT / "security" / "imp-security-optimizer.py")],
        [sys.executable, str(ROOT / "expansion" / "imp-resource-balancer.py")],
    ]
    ...
    for node, cmds in assigned_tasks.items():
        for cmd in cmds:
            if node in LOCAL_HOSTS:
                local_index = tasks.index(cmd)
                subprocess.run(local_tasks[local_index])
            else:
