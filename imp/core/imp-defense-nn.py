"""Neural network specializing in defensive cybersecurity decisions."""
...
class DefenseNN:
    """Simple feedforward neural net used for security actions."""
    ...
    @classmethod
    def load(cls, path: Path) -> "DefenseNN":
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        net.train(sample)
    print("Output:", net.forward([0.0]))
    p = Path("defense_nn.json")
    net.save(p)
    r = DefenseNN.load(p)
    print("Reloaded:", r.forward([0.0]))
    p.unlink()
