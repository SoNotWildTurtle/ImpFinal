...
utils_spec = importlib.util.spec_from_file_location('imp_utils', ROOT / 'core' / 'imp_utils.py')
...
def load_model() -> NetworkTaskNN:
    def factory() -> NetworkTaskNN:
        if MODEL_PATH.exists():
            return NetworkTaskNN.load(MODEL_PATH)
        return NetworkTaskNN(1, 2, 1)
    return neural_manager.get_or_create('network_task', factory)


def latest_ip_count() -> int:
    entries = read_json(DIFF_LOG, [])
    if not entries:
        return 0
